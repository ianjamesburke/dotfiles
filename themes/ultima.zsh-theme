...
# SEGMENT/SSH_STATUS (The Fix)
ssh_marker=""
if [[ -n "$SSH_CLIENT" || -n "$SSH2_CLIENT" || -n "$SSH_TTY" ]]; then
 ssh_marker="%F{white}SSH:%f%F{magenta}%n%f%F{white}@%f%F{yellow}%m%f "
fi
...
# REMOVED FOR MORE TOKEN EFFITIANT COPY PASTING IN TERM
# printPsOneLimiter() {
#   local termwidth
#   local spacing=""
